#!/usr/bin/env python3
"""Static site generator for AI Coding Tools Comparison"""
...
<header>
  <div class="container">
    <a href="/" style="display:flex;align-items:center;gap:12px;text-decoration:none;color:inherit;">
      ...
      <h1 style="font-size:18px;">AI Coding Tools Comparison</h1>
    </a>
    <nav>
      <a href="/">Home</a>
...
def generate_index():
    ...
    for t in tools:
        ...
        <tr>
            ...
            <td>{category_display.get(t['category'], t['category'])}</td>
    ...
    for t in tools:
        ...
        <div class="tool-card">
            ...
            <p style="margin-top:8px;"><span style="color:#f59e0b;">?</span> {t.get('g2_rating', 'N/A') or 'N/A'} ({t.get('g2_reviews', 'N/A')} reviews)</p>
...
<section class="hero">
    <div class="container">
        <h1>AI Coding Tools Pricing Comparison 2026</h1>
        <p>Compare pricing, features, ratings, and capabilities of the top 15 AI coding assistants. Find the perfect tool for your development workflow.</p>
...
<div class="container">
    ...
    <div class="table-container"><table><thead><tr><th>Tool</th><th>Type</th><th>Starting Price</th><th>Free Plan</th><th>Rating</th><th></th></tr></thead><tbody>{overview_rows}</tbody></table></div>
...
def generate_compare_page(tool_a, tool_b):
    ...
    compare_rows = ""
    # Compare prices
    compare_rows += f"<tr><td><strong>Starting Price</strong></td><td>{price_display(a['plans'][1]['price_monthly']) if len(a['plans']) > 1 else 'Free/OSS'}</td><td>{price_display(b['plans'][1]['price_monthly']) if len(b['plans']) > 1 else 'Free/OSS'}</td></tr>"
...
<head>
    ...
    <meta name="google-site-verification" content="B1R0MONqpe21cxbkNvX6UWkEaXdW_xSarjHEY33fg54" />
    <!-- Google Analytics -->
...
def main():
    ...
    for path, pri in [("/", "1.0")] + [(f"/{t['id']}/", "0.9") for t in tools] + [(f"/compare/{a['id']}-{b['id']}/", "0.7") for a,b in pairs] + [(f"/best-{c}-tools/", "0.8") for c in ["ide-assistant", "ai-native-ide", "cli-agent"]]:
...
def generate_category_pages(tools):
    ...
    return ["/best-ide-assistant-tools/", "/best-ai-native-ide-tools/", "/best-cli-agent-tools/"]
